...
public class Part2
{
    ...
    /*
     * had to run with `java -Xss16m Part2` to avoid stack overflow
     * also, don't ask about the runtime
     */
    ...
    private static int findMax(Coord current, boolean[][] visited, int distance) {
        ...
        var neighbours = current.validNeighbours().stream().map(item -> Map.entry(item, 1)).toList();
