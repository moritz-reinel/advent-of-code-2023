...
public class Part2
{
    ...
    public static void main(String[] args) throws IOException
    {
        ...
        int result = 0;
        for (var falling : bricks) {
            Set<Integer> fallen = new HashSet<>();
            fallen.add(falling.id());

            for (var b : bricks) {
                HashSet<Integer> staying = new HashSet<>(b.supporters());
                staying.removeAll(fallen);
                if (b.supporters().size() > 0 && staying.size() == 0) {
                    fallen.add(b.id());
                }
            }
            fallen.remove(falling.id());
